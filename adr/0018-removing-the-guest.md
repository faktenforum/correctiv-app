...
- **Nothing inside the app is locked, and that rule is untouched.** The scope says so
  ...
  neither are the passages in [ADR 0004](0004-react-native-pivot.md) and
  [ADR 0006](0006-one-core-two-hosts.md) that state the rule. They describe the
  interior and the interior has not changed. What became false is the copy that spoke
  about the *offer* — that CORRECTIV's journalism is free for everyone — and that copy
  is listed above as removed. The door is at the entrance, not between the screens.

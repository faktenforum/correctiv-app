    ...
    <GridLayout rows="auto, *" class="bg-grey-100">
      ...
      <ScrollView row="1">
        <StackLayout class="px-sm py-m">
          ...
          <Label
            :text="`„${claim.quote}“`"
            class="ty-headline-m text-grey-700 font-serif-bold mt-s"
            textWrap="true"
          />
          ...
          <GridLayout columns="*, *, *, *, *" class="claim-progress">
            ...
            <StackLayout col="2">
              <Label
                class="claim-progress__dot"
                :class="{ 'claim-progress__dot--done': stage >= 1 }"
              />
            ...
            <StackLayout col="4">
              <Label
                class="claim-progress__dot"
                :class="{ 'claim-progress__dot--final': stage >= 2 }"
              />
          ...
          <StackLayout v-if="claim.sources.length === 0" class="card mt-s">
            <Label
              text="Noch keine Quellen — die Community sammelt."
          ...
          >
            ...
            <StackLayout col="1">
              <Label
                :text="source.note ?? source.url"
                class="ty-text-m text-grey-700"
                textWrap="true"
              />
              <Label
                :text="`Verlässlichkeit: ${source.credibility}`"
          ...
          <Button
            text="Eigenen Hinweis einreichen (im Faktenforum)"
            class="btn-secondary mt-m"
            @tap="openForum"
          />
        </StackLayout>
      </ScrollView>
...
import { claimStatusTag, type Claim, type ClaimSource } from '@correctiv/app-core/data/claims';
...
const stage = computed(() =>
  props.claim.status === 'checked' ? 2 : props.claim.status === 'checking' ? 1 : 0,
);

import { audioStore, PREVIEW_LIMIT_SEC } from '@correctiv/app-core/stores/audio';
import type { AudioTrack } from '@correctiv/app-core/types/models';

/**
 * The app's audio actions.
 *
 * All of the logic — the state machine, the watchdog, the 60-second club preview,
 * the German error copy — is in `@correctiv/app-core/stores/audio`, shared with
 * the NativeScript app. expo-audio sits behind `AudioBackend` in `./backend.ts`.
 *
...
export const playPreview = (track: Omit<AudioTrack, 'kind'>): Promise<void> =>
  audioStore.getState().playPreview(track);
...
export const acknowledgePreviewEnd = (): void => audioStore.getState().acknowledgePreviewEnd();

...
**Das Gate ist gefahren und auf Android bestanden** (ADR 0003). Authentifizierter Stream, Lock-Screen-MediaSession, Hintergrund-Wiedergabe und authentifizierter Download laufen — **ohne natives Modul**, komplett in TypeScript über die von NativeScript exponierte Android-API. Serverseitig bewiesen gegen einen Testserver, der ohne `Authorization`-Header 401 liefert. Damit ist der stärkste verbliebene Wechselgrund („authentifiziertes Hintergrund-Audio geht nur nativ") widerlegt. Der Produktionsweg heißt: `@nativescript-community/audio` ablösen (es kennt keine Header-Option) und `stores/audio.ts` gegen die Plattform-API neu schreiben — der bereits eingeplante ~600-LOC-Rewrite, jetzt mit belegter Machbarkeit. **Offen: iOS** (`AVURLAsset` + `MPNowPlayingInfoCenter`, ungeprüft) — ändert die Empfehlung nicht, wäre bei Expo ebenfalls Arbeit, gehört aber vor die Beta-Planung.
...
2. ✅ **erledigt** — Audio-Spike bestanden (ADR 0003): authentifizierter Stream, MediaSession, Hintergrund-Wiedergabe, authentifizierter Download, alles ohne natives Modul. **Neuer Schritt 2: derselbe Spike für iOS**, sobald ein Mac verfügbar ist.

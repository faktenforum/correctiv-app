...
- **Offline:** the demo must never depend on Wi-Fi. Feeds fall back to bundled
  ...
  and Salon5 podcasts to a bundled snapshot (`src/assets/data/podcasts/`,
  generated by `scripts/fetch-offline-podcasts.mjs`).
- **Search & podcasts (live):** `services/search.service.ts` runs full-text
  search over the public correctiv.org WordPress REST API (`/wp-json/wp/v2/posts`);
  `services/podcast.service.ts` + `stores/podcasts.ts` load the Salon5 shows from
